...
const generateSections = (count, color, rotate) => {
  ...
  const nextColor = color === "#171717" ? "white" : "#171717"; 
...
const Trippy = ({ rotate }) => {
  return (
    <motion.div className="absolute inset-0 overflow-hidden bg-[#171717]"> 
      {generateSections(0, "#171717", rotate)} 
    </motion.div>
  );
};
...
const Section = ({ background, children, rotate }) => {
  return (
    <motion.div
      className="relative min-h-screen w-full origin-center"
      style={{
        background,
        rotate,
        padding: PADDING,
